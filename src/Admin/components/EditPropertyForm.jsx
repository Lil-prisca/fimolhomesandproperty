...
export default function EditPropertyForm({ property }) {
  ...
  const [imagePreviews, setImagePreviews] = useState(
    property.property_images
      ...
      .map((img) => ({ url: img.url, existing: true, id: img.id })) ?? [],
  );
    ...
    files.forEach((file) => {
      ...
      reader.onload = (ev) => {
        setImagePreviews((prev) => [
          ...prev,
          { url: ev.target.result, existing: false },
        ]);
      };
  ...
  async function handleSubmit(e) {
    ...
    try {
      const priceNum =
        parseInt(form.priceRaw.replace(/\D/g, ""), 10) || property.price;
      const priceLabel =
        form.price_label || `₦${priceNum.toLocaleString("en-NG")}`;
      ...
      if (imageFiles.length > 0) {
        ...
        const existingCount = imagePreviews.filter((p) => p.existing).length;
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="glass rounded-2xl p-6 border border-white/10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className={labelClass}>Price (raw number, ₦)</label>
              <input
                type="text"
                value={form.priceRaw}
            ...
            <div>
              <label className={labelClass}>Price Label (display)</label>
              <input
                type="text"
        ...
        <div className="glass rounded-2xl p-6 border border-white/10">
          ...
          <p className="text-white/40 text-xs mb-5">
            First image is the cover photo. Hover and click × to remove images.
          </p>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-3 mb-4">
              {imagePreviews.map((img, i) => (
                ...
                >
                  ...
                  {img.existing && (
                    <span className="absolute bottom-1 left-1 bg-black/60 text-white/60 text-[10px] px-1.5 py-0.5 rounded">
                      Saved
                    </span>
                  )}
                  <button
          ...
          <label className="flex flex-col items-center justify-center h-28 border-2 border-dashed border-white/15 rounded-xl cursor-pointer hover:border-blue-500/50 transition-colors group">
            <svg
              className="w-7 h-7 text-white/25 group-hover:text-blue-400 transition-colors mb-2"
              viewBox="0 0 24 24"
              fill="none"
            >
              <path
